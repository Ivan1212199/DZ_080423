...
def print_recordsOne(file_name: str):
    with open(file_name, 'r', encoding='utf-8') as data:
        for line in data:
            print(*line.split(';'), end="")


def input_recordsOne(file_name: str):
    with open(file_name, 'r+', encoding='utf-8') as data:
        record_id = 0
        for line in data:
            if line != '':
...
def find_recordsOne(file_name: str, char, condition):
    if condition != 'q':
        printed = False
        with open(file_name, 'r', encoding='utf-8') as data:
            for line in data:
...
def check_id_record(file_name: str, text: str):
    ...
    while decision not in ('1', 'q'):
        if decision != '2':
            ...
            find_recordsOne(path, *find_char())
        decision = input(f'Вы знаете id записи которую хотите {text}? 1 - да, 2 - нет, q - выйти\n')
    if decision == '1':
        record_id = input('Введите id, q - выйти\n')
        while not find_recordsOne(file_name, '0', record_id) and record_id != 'q':
            record_id = input('Введите id, q - выйти\n')
        return record_id
    return decision
...
def change_recordsOne(file_name: str):
...
def delete_recordsOne(file_name: str):
...
while action != 'q':
    ...
    if action != 'q':
        if action == '1':
            print_recordsOne(path)
        elif action == '2':
            input_recordsOne(path)
        elif action == '3':
            find_recordsOne(path, *find_char())
        elif action == '4':
            change_recordsOne(path)
        elif action == '5':
            delete_recordsOne(path)